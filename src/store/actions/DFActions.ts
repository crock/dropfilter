...
export enum FilterActionTypes {
	...
	addFilteredDomains = "addFilteredDomains",
...
const actions: Record<FilterActionTypes, IActionHandler> = {
	addUnfilteredDomains: (state: IDropfilter, payload: string[]) => ({
		...state,
		unfilteredDomains: payload
	}),
	addFilteredDomains: (state: IDropfilter, payload: string[]) => ({
		...state,
	...
	setKeywordPosition: (state: IDropfilter, payload: { index: number, position: string }) => {
		const keywords = state.config.keywords.slice()
		keywords[payload.index].position = payload.position as KeywordPosition
		return { ...state, config: { ...state.config, keywords } }
	},
